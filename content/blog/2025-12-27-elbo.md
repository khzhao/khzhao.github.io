...
&= \sum_{i=1}^k ||x||_2^2 \cdot N(0, 1)^2 \\
&= ||x||_2^2 \sum_{i=1}^k \chi^2(1)
\end{align*}$$
In other words:
$$||f(x)||_2^2 \sim \chi^2(k)$$
Knowing this, because $\chi^2$ is sub-Exponential, we can apply a sub-Exponential tail bound to obtain:
$$\begin{align*}
P(||f(x)||_2^2 \notin 1 \pm \epsilon) &= P\left( \left| ||f(x)||_2^2 - 1 \right| \geq \epsilon \right) \leq 2e^{-\frac{\epsilon^2 k}{8}} \\
\end{align*}$$
...
$$\begin{align*}
P(||f(x)||_2^2 \notin 1 \pm \epsilon) \leq \binom{n}{2} 2e^{-\frac{\epsilon^2 k}{8}} \\
\end{align*}$$
...
$$\begin{align*}
\log p(x) &= \mathcal{L}_\theta + D_{KL}(q_\theta(z | x) || p(z | x)) \\
\end{align*}$$
